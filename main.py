...
def checkExtraMethod():
    ...
    try:
        ...
        print(Fore.MAGENTA + 'Con Cặc đi xem hentai rồi sục đi đợi load xong...')
...
def countdown(t):
    ...
    while True:
        if (until - datetime.datetime.now()).total_seconds() > 0:
            stdout.flush()
            stdout.write("\r " + Fore.MAGENTA + "[*]" + Fore.GREEN + " Attack status => " + str(
                (until - datetime.datetime.now()).total_seconds()) + " sec left ")
        else:
            stdout.flush()
            stdout.write(
                "\r " + Fore.MAGENTA + "[*]" + Fore.RED + " Attack Done !                                   \n")
            return
...
def inputf():
    printf(Colorate.Vertical(Colors.green_to_red, '''
  ╔═══[Karma@Free]
  ╚══> ''', True), end=' ')
  ...
  while True:
    ...
    if 'credit' in cmdl:
      ...
      ╔═════════╩═════════════════════════════════╩═════════╗
        ...
        - Bản quyền            : Karma(Hoang Quy Huy)
        - ZALO                 : 0398242945
        - FACEBOOK             : huy.ta.792
                   CẢM ƠN ĐÃ SỬ DỤNG Karma DDOS
 ...
 - pxslow  | Ddos bằng proxy làm chậm web
  ...
  -tcpc    | Ddos IP TCP tùy chỉnh
      ...
      '''), True), end='')
        ...
        global ip
        print(Back.GREEN+"IP của bạn: {}".format(ip)+Style.RESET_ALL)
        print(Back.GREEN+"IP của bạn đã được gửi tới server." + Style.RESET_ALL)    
